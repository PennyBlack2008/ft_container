  ...
  * Allocator -> 벡터의 메모리 할당 및 할당 취소에 대한 세부 정보를 캡슐화하는 할당자 개체(선택사항)
  ...
  public:
    ...
    */
    ...
    ~vector()
    {
      ft::Destroy(this->M_impl.M_start,
                  this->M_impl.M_finish, M_get_T_allocator());
    }
    ...
    */
    ...
    void swap(vector& x)
    {
      ...
      std::__swap_allocator(this->M_get_T_allocator(), x.M_get_T_allocator());
    }
    ...
    if (&x != this)
    {
      ...
      else if (size() >= xlen)
      {
        ft::Destroy(std::copy(x.begin(), x.end(), begin()),
                    end(), M_get_T_allocator());
      }
      else
      {
        std::copy(x.M_impl.M_start,
                  x.M_impl.M_start + size(), this->M_impl.M_start);
        std::uninitialized_copy(x.M_impl.M_start + size(),
                                x.M_impl.M_finish, this->M_impl.M_finish);
      }
  ...
  template <typename T, typename Alloc>
  void
    vector<T, Alloc>::M_fill_assign(size_t n, const value_type& val)
  {
    if (n > capacity())
    {
  ...
  template<typename InputIterator>
  void
    vector<T, Alloc>::M_assign(InputIterator first, InputIterator last, std::input_iterator_tag)
  {
  ...
  void vector<T, Alloc>::
  M_range_insert(iterator pos, InputIterator first, InputIterator last, std::input_iterator_tag)
  {
    for (; first != last; ++first) {
  ...
  template<typename ForwardIterator>
  void vector<T, Alloc>::
  M_range_insert(iterator position, ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
  {
    if (first != last)
    {
  ...
  M_fill_insert(iterator position, size_type n, const value_type& x)
  {
    if (n != 0)
    {
      ...
      else
      {
        ...
        try
        {
          new_finish = ft::uninitialized_copy_a(this->M_impl.M_start,
                                                position.base(), new_start, M_get_T_allocator());
  ...
  template<typename T, typename Alloc>
  void vector<T, Alloc>::
  M_insert(iterator position, const T& x)
  {
    if (this->M_impl.M_finish != this->M_impl.M_end_of_storage)
    {
      ...
      std::copy_backward(position.base(),
                         this->M_impl.M_finish - 2, this->M_impl.M_finish - 1);
      *position = x_copy;
    }
    else
    {
      ...
      {
        new_finish =
        ft::uninitialized_copy_a(this->M_impl.M_start, position.base(),
                                 new_start, M_get_T_allocator());
