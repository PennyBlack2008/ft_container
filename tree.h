...
namespace ft
{
  ...
  protected:
    ...
    Link_type
      M_create_node(const value_type& x)
      {
        ...
        try
        { std::_Construct(&tmp->M_value_field, x); }
        catch(...)
        { M_put_node(tmp); __throw_exception_again; }
        return tmp;
      }
    ...
    void
      ...
      {
        std::Destroy(&p->M_value_field);
        M_put_node(p);
      }

  protected:
    template<typename Key_compare,
             bool Is_pod_comparator = std::is_pod<Key_compare>::M_type>
      struct RB_tree_impl : public Node_allocator
      {
        ...
        RB_tree_impl(const Node_allocator& a = Node_allocator(),
          ...
          : Node_allocator(a), M_key_compare(comp), M_node_count(0)
        {
  ...
  public:
    ...
    : M_impl(x.get_allocator(), x.M_impl.M_key_compare)
    {
    	if (x.M_root() != 0)
  	  {
  ...
  insert_unique(iterator position, const Val& v)
  {
    if (position.M_node == M_leftmost())
    ...
    else if (position.M_node == M_end())
	  {
	    // end()
	    if (M_impl.M_key_compare(S_key(M_rightmost()),
	                  			     KeyOfValue()(v)))
    ...
    else
	  {
	    ...
	    --before;
	    if (M_impl.M_key_compare(S_key(before.M_node),
				                       KeyOfValue()(v))
	        && M_impl.M_key_compare(KeyOfValue()(v),
				                        	S_key(position.M_node)))
	    {
    ...
    if (position.M_node == M_leftmost())
  	{
  	  // begin()
  	  if (size() > 0
  	      && !M_impl.M_key_compare(S_key(position.M_node),
  					                       KeyOfValue()(v)))
  	    return M_insert(position.M_node, position.M_node, v);
  	  // first argument just needs to be non-null
  	  else
  	...
  	{
  	  // end()
  	  if (!M_impl.M_key_compare(KeyOfValue()(v),
  				                      S_key(M_rightmost())))
  	    return M_insert(0, M_rightmost(), v);
  	  else
  	    return insert_equal(v);
  ...
  RB_tree<Key,Val,KeyOfValue,Compare,Alloc>::erase(const Key& x)
  {
  ...
  template<typename Key, typename Val, typename KeyOfValue,
           typename Compare, typename Alloc>
  void
    RB_tree<Key,Val,KeyOfValue,Compare,Alloc>::M_erase(Link_type x)
  {
  ...
  template<typename Key, typename Val, typename KoV,
           typename Compare, typename Alloc>
  inline
  pair<typename RB_tree<Key, Val, KoV,
