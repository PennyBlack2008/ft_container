...
public:
    ...
    void push_back(T s)
    {
        if (capacity <= length)
        {
            T* temp = new T[capacity * 2]; // 새 Vector 할당
            for (int i = 0; i < length; i++)
                temp[i] = data[i];
            delete[] data; // Vector 삭제
            data = temp; 
            capacity *= 2;
        }
        data[length] = s;
...
/*
Q 왜 하필이면 bool type 은 따로 선언했을 까?
- A capacity 를 int 로 선언했기 때문에 위의 template 을 쓰면, 2147483647 bool 갯수까지
    체크가 가능하다. 하지만, int 형을 bit 단위로 쪼개면 bool type 을 2147483648 * 2 * 32 갯수까지
    커버가 가능하다.
*/
template <>
class Vector<bool>
